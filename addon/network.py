...
class ComfyDeployClient:
    ...
    async def upload_image(self, image_data: bytes, image_size: str) -> None:
        ...
        if upload_url_result is not None:
            ...
            upload_request = requests.put(upload_url_result.upload_url, json=image_data, headers={
                "Content-Type": operations.Type.APPLICATION_OCTET_STREAM
            })
    ...
    async def run_workflow(self, prompt: str, prompt_a: str, prompt_b: str, prompt_c: str, prompt_d: str) -> str:
        if self.mask and self.depth and self.outline:
            logging.info(f"Starting workflow for prompt: {prompt} "
                         f"with detailed prompt {prompt_a}, {prompt_b}, {prompt_c}, {prompt_d}")
            run_result = self.client.run.create(request={
